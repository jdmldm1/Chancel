...
export const adminResolvers = {
    Query: {
        ...
        allSessions: async (_parent, _args, context) => {
            const userId = requireAdmin(context);
            const user = await context.prisma.user.findUnique({
                where: { id: userId },
            });
        ...
        allGroups: async (_parent, _args, context) => {
            const userId = requireAdmin(context);
            const user = await context.prisma.user.findUnique({
                where: { id: userId },
            });
        ...
        adminStats: async (_parent, _args, context) => {
            ...
            const [totalUsers, totalLeaders, totalMembers, totalSessions, totalGroups, totalComments, totalPrayerRequests,] = await Promise.all([
    ...
    Mutation: {
        adminDeleteUser: async (_parent, args, context) => {
        ...
        adminUpdateUserRole: async (_parent, args, context) => {
            const adminId = requireAdmin(context);
            const admin = await context.prisma.user.findUnique({
                where: { id: adminId },
            });
        ...
        adminDeleteSession: async (_parent, args, context) => {
            const adminId = requireAdmin(context);
            const admin = await context.prisma.user.findUnique({
                where: { id: adminId },
            });
        ...
        adminDeleteGroup: async (_parent, args, context) => {
            const adminId = requireAdmin(context);
            const admin = await context.prisma.user.findUnique({
                where: { id: adminId },
            });
        ...
        adminDeleteSeries: async (_parent, args, context) => {
            const adminId = requireAdmin(context);
            const admin = await context.prisma.user.findUnique({
                where: { id: adminId },
            });
